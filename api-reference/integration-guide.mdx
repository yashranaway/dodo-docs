...
- `redirect_url`: The URL to redirect the customer to after the payment is completed.
    ...
    **Note:** Dodo will also attach the payment id and status as query params when redirecting the customer to this URL.
    For example, If the redirect url provided is https://example.com then the below would be the redirected URL for a Successful payment.
