...
<Frame>
<img src="/images/cover-images/subscription-upgrade-downgrade.png" alt="Subscription upgrade and downgrade flow diagram showing plan changes and billing adjustments"/>
</Frame>
...
<Tabs>
<Tab title="Fair Billing - prorated_immediately">
Best for: SaaS applications wanting to charge fairly for unused time
- Calculates exact prorated amount based on remaining cycle time
- Charges only the difference between plans
- Provides transparent billing to customers
</Tab>

<Tab title="Simple Upgrade - difference_immediately">
...
<Tab title="Reset Billing - full_immediately">
...
<Tabs>
  <Tab title="Next.js Route Handler">
    ...
    export async function POST(req) {
      ...
      // verifySignature is a placeholder – use your SDK/helper
      const { valid, event } = await verifySignature(payload, signature, secret);
  ...
  <Tab title="Express.js">
    ...
    app.post('/webhooks/dodo', express.raw({ type: 'application/json' }), async (req, res) => {
      const signature = req.header('x-dodo-signature');
      const secret = process.env.DODO_WEBHOOK_SECRET;
...
**Common causes**:
- Using wrong proration mode (credits only apply with `difference_immediately`)
