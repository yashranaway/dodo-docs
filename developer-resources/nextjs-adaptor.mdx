...
<Card title="Checkout Sessions" icon="credit-card" href="#checkout-sessions-post">
    Create structured checkout experiences with pre-configured carts.
</Card>
</CardGroup>
...
Checkout sessions provide a more secure, hosted checkout experiences that handle the complete payment flow for both one-time purchases and subscriptions with full customization control. You can read more about checkout sessions [here](https://docs.dodopayments.com/developer-resources/checkout-session).
...
returnUrl: process.env.DODO_PAYMENTS_RETURN_URL,
environment: process.env.DODO_PAYMENTS_ENVIRONMENT,
type: "session", // or "session" for checkout sessions
});
