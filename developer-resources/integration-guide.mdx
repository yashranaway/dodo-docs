...
Dodo Payments supports three types of payment links for integrating payments into your website:
...
- **Checkout Sessions**: Create secure, hosted checkout experiences that handle the complete payment flow with enhanced customization.
...
Checkout Sessions provide a modern, secure approach to creating hosted checkout experiences. Unlike payment links, checkout sessions offer enhanced control over the checkout flow and require minimal customer information upfront.

<Card title="Why Choose Checkout Sessions?">
  - **Flexible data collection**: Provide only the information you have, we'll collect the rest
  - **Better UX**: Streamlined checkout flow with automatic data validation
  - **Full customization**: Control payment methods, themes, and features
</Card>
  ...
  <Tab title="Node.js SDK">
    ...
    async function createCheckoutSession() {
      const session = await client.checkoutSessions.create({
        ...
        product_cart: [
          {
            product_id: 'prod_123', // Replace with your product ID
            quantity: 1
          }
        ],
        ...
        // Billing address (optional - we'll collect missing details)
        billing_address: {
          street: '123 Main St',
          city: 'San Francisco',
          state: 'CA',
...
<AccordionGroup>
  <Accordion title="Flexible Data Collection">
    Unlike Dynamic Payment Links, you don't need complete billing information upfront. Provide what you have, and the checkout will collect the rest:
  ...
  <Accordion title="Payment Method Control">
    Restrict available payment methods for your specific use case:
  ...
  <Accordion title="Instant Confirmation">
    Skip data collection and go straight to payment when you have complete information:
    ...
    const session = await client.checkoutSessions.create({
      ...
      confirm: true, // Skip data collection, go straight to payment
      return_url: 'https://yoursite.com/success'
    });
    ```
  </Accordion>
...
<Info>
For comprehensive documentation on all available options, see our [Checkout Sessions Guide](/developer-resources/checkout-session).
</Info>
