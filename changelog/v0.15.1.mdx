...
### API Enhancements
- Launched Invoice API enabling merchants and customers to download invoices via `payment_id`. Visit [Invoice API](/api-reference/payments/get-invoice) documentation for more details.
...
- Published comprehensive guides:
    - [Subscription Integration Guide](/api-reference/subscription-integration-guide)
    - [Webhook Events Guide](/api-reference/webhooks/intents/webhook-events-guide)
...
- Refined customer identification logic: now using consistent `customer_id` across payments/subscriptions for the same email
