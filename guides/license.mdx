...
<Info>Ensure keys are unique and hard to guess.</Info>

b. **Use a License Key Generation Library/Tool:**
...
c. **Trigger Key Generation via Dodo Payments Webhook:**

* Once **Dodo Payments** confirms a successful transaction via webhook, generate the license key programmatically.
...
d. **Store Keys Securely:**

Save generated keys in a database with the following fields (Recommended):

| Customer Email       | Product Name    | Product Quantity | Purchase Date | License Key         | Expiry Date  |
| -------------------- | --------------- | ---------------- | ------------- | ------------------- | ------------ |
| john.doe@example.com | Example Product | 1                | 2023-10-01    | ABCD-EFGH-IJKL-MNOP | 2024-10-01   |
...
<Card title="Download Page" icon="download">
...
The following steps are recommended by dodopayments to manage license keys effectively:

a. **Track Usage:**

* Limit the number of devices that can use a key.

Example: Store the device's unique identifier (e.g., MAC address, IMEI) alongside the license key in your database.

b. **Revoke Keys:**

* Use Dodo Payments' transaction webhooks to identify refunds or disputes and revoke the associated keys.

c. **Renewals (For Subscriptions):**
...
* Dodo Payments notifies merchants instantly of successful transactions via:
  ...
  * **Emails**: Automated notifications containing transaction details.

  * **Webhooks**: Immediate confirmation of payments.
...
By integrating **Dodo Payments** webhhoks with your license key system, you can automate key generation, validation, and delivery for a seamless customer experience.